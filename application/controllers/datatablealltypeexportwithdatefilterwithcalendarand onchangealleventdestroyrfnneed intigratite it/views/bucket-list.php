			...
			<div class="col-md-10">
				...
				<section class="invoice show">
					...
					<div class="row" style="background-color: #587EA3">
						<div class="col-md-12">
							<h2 class="lel">Bucket List</h2> </div>
					</div>
					...
					<div class="row">
					    ...
					    ?>
						<a href="<?php echo base_url()?>create-bucket">
				...
				<?php
					$a=1;
					foreach ($bucketlist as $bucket_list) {
				?>
				<tr>
					<td><?php echo $a ?>.</td>
					<td><?php echo $bucket_list['bucket_question'] ?></td>
					<td><?php $b=1; $opt = explode('|', $bucket_list['bucket_option']); foreach ($opt as $opts) {
						if($opts != ''){
							echo $b . '. ' .$opts . '<br />'; $b++;
						}
					} ?></td>
					<td><?php $c=1; $ca = explode('@|', $bucket_list['bucket_answer']); foreach ($ca as $cas) {
						echo $c . '. ' .$cas . '<br />'; $c++;
					} ?></td>
					<td><?php 
						$topicid = $bucket_list['topic'];
						$where = "t_id='$topicid'";
						$rval = $this->Crud_modal->fetch_single_data('t_name','master_topic',$where);
						echo $rval['t_name'] ?></td>
					<td><?php
						$typeid = $bucket_list['type'];
						$where = "type_id='$typeid'";
						$rval = $this->Crud_modal->fetch_single_data('type_name','master_type',$where);
						echo $rval['type_name']; ?></td>
					<td><?php 
						$skillid = $bucket_list['skill_tested'];
					...
					<td>
					    ...
					    ?>
						<a href="<?php echo base_url().'bucket-view/'.rtrim(strtr(base64_encode($bucket_list['bucket_id']), '+/', '-_'), '='); ?>">&nbsp;View&nbsp;</a>
						<?php }if($p["permission_description"]=="Edit"){ ?>
						<a href="<?php echo base_url().'bucket-edit/'.rtrim(strtr(base64_encode($bucket_list['bucket_id']), '+/', '-_'), '='); ?>">&nbsp;Edit&nbsp;</a>
						<?php }if($p["permission_description"]=="Delete"){ ?>
						<a href="<?php echo base_url().'bucket-delete/'.rtrim(strtr(base64_encode($bucket_list['bucket_id']), '+/', '-_'), '='); ?>">&nbsp;Delete&nbsp;</a>
						<?php }} ?>
					</td>
				</tr>
