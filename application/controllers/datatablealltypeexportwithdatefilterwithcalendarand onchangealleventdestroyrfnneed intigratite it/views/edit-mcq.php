					...
					<div class="col-md-12">
						<section class="content">
							...
							<div class="box box-col">
								...
								<!-- /.box-header -->
								<div class="box-body">
									<form autocomplete="off" method="post" action="<?php echo base_url() ?>mcq-update">
										<div class="row">
											<div class="col-md-12">
												<div class="form-group">
													<textarea id="editor" name="question" placeholder="Question" class="form-control mcq_question" style="height: 100px;"><?php echo $mcq['question'] ?></textarea>
												</div>
											<!-- /.form-group -->
											</div>
											<?php
												$options = explode('sohrab', $mcq['options']);
												$c_answer = explode('sohrab', $mcq['c_answer']);
												$a=0;
											...
											<div class="col-md-12" style="margin-bottom: 10px;">
												<div class="form-group">
													...
													?>
													<input type="checkbox" name="canswer[]" class="canswer" <?php echo $ck1; ?> value="<?php echo $ck; ?>" style="float: left; width: 5%; height: 30px;" />
													<input type="text" maxlength="70" name="opt[]" value="<?php echo $option; ?>" class="form-control" style="float: left; width: 95%;" />
												</div>
											</div>
											<?php
												}
													...
													<select name="master_topic" class="form-control">
														...
														<?php
															foreach ($master_topics as $master_topic) {
																if($master_topic['t_id'] == $mcq['topic']){
													...
													<select name="master_type" class="form-control">
														...
														<?php
															foreach ($master_types as $master_type) {
																if($master_type['type_id'] == $mcq['type']){
													...
													<select name="master_skills_test" class="form-control">
														...
														<?php
															foreach ($master_skills_tests as $master_skills_test) {
																if($master_skills_test['skills_id'] == $mcq['skill_tested']){
													...
													<select name="master_case" class="form-control" required="required">
														...
														<?php
															foreach ($master_case as $master_cases) {
																if($master_cases['case_id'] == $mcq['case_id']){
													...
													<select name="master_difficulty_level" class="form-control" required="required">
														...
														<?php
															foreach ($master_difficulty as $master_difficulty_level) {
																if($master_difficulty_level['diffi_id'] == $mcq['difficulty_level']){
																	$skillsel = ' selected="selected"';
																}else{
																	$skillsel = '';
																}
											...
											<div class="col-md-12" style="margin-bottom: 10px;">
												<div class="form-group">
													<input type="text" name="timerange" id="timeformatExample1" value="<?php echo $mcq['mcq_time'] ?>" class="form-control">
												</div>
											</div>
										<!-- /.col -->
										</div>
										<input type="hidden" name="mcq_id" value="<?php echo $mcq['mcq_id'] ?>" />
