...
<div class="content-wrapper">
	...
	<div class="row main">
		<div class="col-md-12">
			...
			<div class="col-md-10">
				<?php
				if($this->session->flashdata('bucket_message')){
					echo $this->session->flashdata('bucket_message');
				}
				?>
				<section class="invoice show">
					...
					<div class="row" style="background-color: #587EA3">
						<div class="col-md-12">
							<h2 class="lel">Auto Submit List</h2> </div>
					</div>
					...
					<div class="row">
						<div class="col-md-12">
							...
							<div class="box-header">
					            <div class="col-md-12">
									...
									<?php }} ?>
							<form action="<?php echo base_url()?>bucket-list" method="post" id="filtercheck">
				        	<div class="col-md-3">
					    ...
					    ?>
						<a href="<?php echo base_url()?>auto-submit-create">
						<button type="button" class="btn btn-primary btn-md" style="float: right;background-color:#112B4E; border-color: #112B4E; margin-left: 3px; ">Create Auto Submit</button></a>
						<?php }} ?>
					</div>
               ...
               <div class="table-responsive">
				...
				<?php
					$a=1;
					foreach ($auto_submit as $auto_submit_detail) {
				?>
				<tr>
					<td><?php echo $a ?>.</td>
					<td><?php echo $auto_submit_detail['question_context'] ?></td>
					<td><?php echo $auto_submit_detail['case_title'] ?></td>
					<td><?php echo $auto_submit_detail['question_details'] ?></td>
					<td><?php echo $auto_submit_detail['question_heading'] ?></td>
					<td><?php 
						$topicid = $auto_submit_detail['topic'];
						$where = "t_id='$topicid'";
						$rval = $this->Crud_modal->fetch_single_data('t_name','master_topic',$where);
						echo $rval['t_name'] ?></td>
					<td><?php
						$typeid = $auto_submit_detail['type'];
						$where = "type_id='$typeid'";
						$rval = $this->Crud_modal->fetch_single_data('type_name','master_type',$where);
						echo $rval['type_name']; ?></td>
					<td><?php 
						$skillid = $auto_submit_detail['skill_tested'];
					...
					<td><?php 
						$caseid = $auto_submit_detail['case_id'];
						$where = "case_id='$caseid'";
						$rval = $this->Crud_modal->fetch_single_data('case_name','case_study',$where);
						echo $rval['case_name'] ?></td>
					<td><?php echo  date('d-m-Y',strtotime($auto_submit_detail['created_date'])) ?></td>
					<td>
						<?php 
							  for($i=0;$i<sizeof($permission_array);$i++){
							    $p =$this->Crud_modal->fetch_single_data('*','master_permission',"permission_id='$permission_array[$i]'");
					    ...
					    ?>
						<!-- <a href="<?php echo base_url().'bucket-view/'.rtrim(strtr(base64_encode($auto_submit_detail['auto_submit_id']), '+/', '-_'), '='); ?>">&nbsp;View&nbsp;</a> -->
						<?php }if($p["permission_description"]=="Edit"){ ?>
						<a href="<?php echo base_url().'edit-auto-submit/'.rtrim(strtr(base64_encode($auto_submit_detail['auto_submit_id']), '+/', '-_'), '='); ?>">&nbsp;Edit&nbsp;</a>
						<?php }if($p["permission_description"]=="Delete"){ ?>
						 <a href="<?php echo base_url().'delete-auto-submit/'.rtrim(strtr(base64_encode($auto_submit_detail['auto_submit_id']), '+/', '-_'), '='); ?>" class="delete">&nbsp;Delete&nbsp;</a>
						<?php }} ?>
					</td>
				</tr>
