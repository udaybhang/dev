			...
			<div class="col-md-10">
				...
				<section class="invoice show">
					...
					<div class="row">
						...
						<?php }} ?>
							<form action="<?php echo base_url()?>mcq-list" method="post" id="filtercheck">
				        	<div class="col-md-3">
						...
						?>
						<a href="<?php echo base_url()?>create-mcq">
						<button type="button" class="btn btn-primary btn-md" style="float: right;background-color:#112B4E; border-color: #112B4E; margin-left: 3px; ">Create MCQ</button></a>
						<?php }} ?>
					</div>
				...
				<?php
					$a=1;
					foreach ($msqlist as $msqlists) {
				?>
				<tr>
					<td><?php echo $a ?>.</td>
					<td><?php echo $msqlists['question'] ?></td>
					<td><?php $b=1; $opt = explode('sohrab', $msqlists['options']); foreach ($opt as $opts) {
						if($opts != ''){
							echo $b . '. ' .$opts . '<br />'; $b++;
						}
					} ?></td>
					<td><?php $c=1; $ca = explode('sohrab', $msqlists['c_answer']); foreach ($ca as $cas) {
						echo $c . '. ' .$cas . '<br />'; $c++;
					} ?></td>
					<td><?php 
						$topicid = $msqlists['topic'];
						$where = "t_id='$topicid'";
						$rval = $this->Crud_modal->fetch_single_data('t_name','master_topic',$where);
						echo $rval['t_name'] ?></td>
					<td><?php
						$typeid = $msqlists['type'];
						$where = "type_id='$typeid'";
						$rval = $this->Crud_modal->fetch_single_data('type_name','master_type',$where);
						echo $rval['type_name']; ?></td>
					<td><?php 
						$skillid = $msqlists['skill_tested'];
					...
					<td>
						...
						?>
						<a href="<?php echo base_url().'mcq-view/'.rtrim(strtr(base64_encode($msqlists['mcq_id']), '+/', '-_'), '='); ?>">&nbsp;View&nbsp;</a>
						<?php }if($p["permission_description"]=="Edit"){ ?>
						<a href="<?php echo base_url().'mcq-edit/'.rtrim(strtr(base64_encode($msqlists['mcq_id']), '+/', '-_'), '='); ?>">&nbsp;Edit&nbsp;</a>
						<?php }if($p["permission_description"]=="Delete"){ ?>
						<a href="<?php echo base_url().'mcq-delete/'.rtrim(strtr(base64_encode($msqlists['mcq_id']), '+/', '-_'), '='); ?>">&nbsp;Delete&nbsp;</a>
						<?php }} ?>
					</td>
				</tr>
