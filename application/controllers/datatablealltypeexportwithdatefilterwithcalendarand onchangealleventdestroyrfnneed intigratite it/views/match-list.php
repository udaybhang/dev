			...
			<div class="col-md-1"></div>
				...
				<section class="invoice show">
					...
					<div class="row" style="background-color: #587EA3">
						<div class="col-md-12">
							<h2 class="lel">Match the following Question List</h2> </div>
					</div>
					...
					<div class="row">
						...
						<?php }} ?>
							<form action="<?php echo base_url()?>match-list" method="post" id="filtercheck">
				        	<div class="col-md-3">
						...
						?>
						<a href="<?php echo base_url()?>match-the-following">
						<button type="button" class="btn btn-primary btn-md" style="float: right;background-color:#112B4E; border-color: #112B4E; margin-left: 3px; ">Create Match the following Question List
						</button></a>
						<?php }} ?>
					</div>
				...
				<tr>
					<td><?php echo ++$a ?>.</td>
					<td><?php echo $match_list['question_heading']; ?></td>
					<td><?php echo $match_list['question_topic']; ?></td>
					<td><?php 
						$topicid = $match_list['topic'];
						$where = "t_id='$topicid'";
						$rval = $this->Crud_modal->fetch_single_data('t_name','master_topic',$where);
						echo $rval['t_name'] ?></td>
					<td><?php
						$typeid = $match_list['type'];
						$where = "type_id='$typeid'";
						$rval = $this->Crud_modal->fetch_single_data('type_name','master_type',$where);
						echo $rval['type_name']; ?></td>
					<td><?php 
						$skillid = $match_list['skill_tested'];
					...
					foreach ($options as $value) {
					  ...
					  ?></td>
					  <td>
					<?php  	$new_options =explode('comma',$match_list['match_answer']);
					$c=0;
					foreach ($new_options as $new_value) {
					 	echo ++$c.'. '.$new_value.'<br />';
					 } ?>
					  </td>
					<td><?php echo  date('d-m-Y H:i:s',strtotime($match_list['created_date'])) ?></td>
					...
					<td>
						...
						?>
						<a href="<?php echo base_url().'match-view/'.rtrim(strtr(base64_encode($match_list['match_id']), '+/', '-_'), '='); ?>">&nbsp;View&nbsp;</a>
						<?php }if($p["permission_description"]=="Edit"){ ?>
						<a href="<?php echo base_url().'match-update-view/'.rtrim(strtr(base64_encode($match_list['match_id']), '+/', '-_'), '='); ?>">&nbsp;Edit&nbsp;</a>
						<?php }if($p["permission_description"]=="Delete"){ ?>
						<a href="<?php echo base_url().'delete-match/'.rtrim(strtr(base64_encode($match_list['match_id']), '+/', '-_'), '='); ?>">&nbsp;Delete&nbsp;</a>
						<?php }} ?>
					</td>
				</tr>
