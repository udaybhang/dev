...
<div class="content-wrapper">
	...
	<div class="row main">
		<div class="col-md-12">
			...
			<div class="col-md-10">
				<?php
				if($this->session->flashdata('word_detective_message')){
					echo $this->session->flashdata('word_detective_message');
				}
				?>
				<section class="invoice show">
					...
					<div class="row" style="background-color: #587EA3">
						<div class="col-md-12">
							<h2 class="lel">Word Detective List</h2> </div>
					</div>
					...
					<div class="row">
						...
						<?php }} ?>
							<form action="<?php echo base_url()?>word-detective-list" method="post" id="filtercheck">
				        	<div class="col-md-3">
					    ...
					    ?>
						<a href="<?php echo base_url()?>create-word-detective">
						<button type="button" class="btn btn-primary btn-md" style="float: right;background-color:#112B4E; border-color: #112B4E; margin-left: 3px; ">Create Bucket</button></a>
						<?php }} ?>
					</div>
				...
				<?php
					$a=1;
					foreach ($word_detective_list as $worddetective_list) {
				?>
				<tr>
					<td><?php echo $a ?>.</td>
					<td><?php echo $worddetective_list['word_detective_question'] ?></td>
					<td><?php 
					$s=1;
						$worddetective_hint = explode('|', $worddetective_list['word_detective_hint']);
						 
						 ?></td>
					<td><?php $c=1; $ca = explode('@|', $worddetective_list['word_detective_answer']); foreach ($ca as $cas) {
						echo $c . '. ' .$cas . '<br />'; $c++;
					} ?></td>
					<td><?php 
						$topicid = $worddetective_list['topic'];
						$where = "t_id='$topicid'";
						$rval = $this->Crud_modal->fetch_single_data('t_name','master_topic',$where);
						echo $rval['t_name'] ?></td>
					<td><?php
						$typeid = $worddetective_list['type'];
						$where = "type_id='$typeid'";
						$rval = $this->Crud_modal->fetch_single_data('type_name','master_type',$where);
						echo $rval['type_name']; ?></td>
					<td><?php 
						$skillid = $worddetective_list['skill_tested'];
					...
					<td><?php 
						$caseid = $worddetective_list['case_id'];
						$where = "case_id='$caseid'";
						$rval = $this->Crud_modal->fetch_single_data('case_name','case_study',$where);
						echo $rval['case_name'] ?></td>
					<td><?php echo  date('d-m-Y',strtotime($worddetective_list['created_date'])) ?></td>
					<td>
						<?php 	
								for($i=0;$i<sizeof($permission_array);$i++){
									$p =$this->Crud_modal->fetch_single_data('*','master_permission',"permission_id='$permission_array[$i]'");
						...
						?>
						<a href="<?php echo base_url().'word-detective-view/'.rtrim(strtr(base64_encode($worddetective_list['word_detective_id']), '+/', '-_'), '='); ?>">&nbsp;View&nbsp;</a>
						<?php }if($p["permission_description"]=="Edit"){ ?>
						<a href="<?php echo base_url().'word-detective-edit/'.rtrim(strtr(base64_encode($worddetective_list['word_detective_id']), '+/', '-_'), '='); ?>">&nbsp;Edit&nbsp;</a>
						<?php }if($p["permission_description"]=="Delete"){ ?>
						<a href="<?php echo base_url().'word-detective-delete/'.rtrim(strtr(base64_encode($worddetective_list['word_detective_id']), '+/', '-_'), '='); ?>">&nbsp;Delete&nbsp;</a>
						<?php }} ?>
					</td>
				</tr>
