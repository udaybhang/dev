		...
		<div class="col-md-12">
			...
			<section class="invoice show">
				...
				<div class="row" style="background-color: #587EA3">
					<div class="col-md-12">
					<h2 class="lel">Grid Fib Question</h2> </div>
				</div>
						...
						<section class="content">
							...
							<div class="box box-col">
								<div class="box-header with-border">
									<h3 class="box-title">Grid Fib Question</h3>
								</div>
								<!-- /.box-header -->
								<div class="box-body">
									<form autocomplete="off" method="post" action="<?php echo base_url()?>update-grid-fib">
										<div class="row">
											<div class="col-md-12">
												<div class="form-group">
													<textarea id="editor" name="question" class="form-control mcq_question"><?php echo $grid_fib_edit_data[0]['fib_question']; ?></textarea>
												</div>
											</div>
											<?php
											$options =explode(',', $grid_fib_edit_data[0]['fib_answer']);
											$a=0;
											while($a < 9){
											if($a<sizeof($options)){
												if($options[$a]!=''){
													...
													<select name="master_topic" class="form-control">
														...
														<?php
															foreach ($master_topics as $master_topic) {
																if($master_topic['t_id'] == $grid_fib_edit_data[0]['topic']){
													...
													<select name="master_type" class="form-control">
														...
														<?php
															foreach ($master_types as $master_type) {
																if($master_type['type_id'] == $grid_fib_edit_data[0]['type']){
													...
													<select name="master_skills_test" class="form-control">
														...
														<?php
															foreach ($master_skills_tests as $master_skills_test) {
																if($master_skills_test['skills_id'] == $grid_fib_edit_data[0]['skill_tested']){
													...
													<select name="master_case" class="form-control" required="required">
														...
														<?php
															foreach ($master_case as $master_cases) {
																if($master_cases['case_id'] == $grid_fib_edit_data[0]['case_id']){
													...
													<select name="master_difficulty_level" class="form-control" required="required">
														...
														<?php
															foreach ($master_difficulty as $master_difficulty_level) {
																if($master_difficulty_level['diffi_id'] == $grid_fib_edit_data[0]['difficulty_level']){
																	$skillsel = ' selected="selected"';
																}else{
																	$skillsel = '';
																}
										...
										<input type="hidden" name="g_id" value="<?php echo rtrim(strtr(base64_encode($grid_fib_edit_data[0]['fib_id']), '+/', '-_'), '='); ?>" />
										<a href="<?php echo base_url()?>grid-fib-list" class="btn btn-primary btn-md " style="float: left;background-color:#112B4E; border-color: #112B4E; margin-top: 20px;">Back</a>
