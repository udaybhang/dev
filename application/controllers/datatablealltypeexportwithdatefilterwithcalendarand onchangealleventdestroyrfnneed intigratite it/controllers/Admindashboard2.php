  ...
  function __construct()
   ...
   {   		
		if(($this->session->userdata('admin_name')!="" || $this->session->userdata('admin_name')!=null) && ($this->session->userdata('admin_role')!="" || $this->session->userdata('admin_role')!=null)){
			...
			if($user_id!=''){
				 $data['asid']=$user_id;
				$data['msqlist'] = $this->Crud_modal->all_data_select('*','mm_master_job',"user_type_id='$user_id'",'job_id desc');
			}else{
				
				$data['msqlist'] = $this->Crud_modal->fetch_alls('mm_master_job','job_id desc');
			}
			$this->load->view('admintempletes/head',$data);
			$this->load->view('admintempletes/header',$data);	
			$this->load->view('admintempletes/sidebar',$data);
	 	...
	 	try{
	 		if($this->session->userdata('admin_name')!="" || $this->session->userdata('admin_name')!=null){
				...
				$leader_name=$this->Crud_modal->fetch_single_data("mm_user_full_name","user","mm_uid='$group_name[group_leader_uid]'");
	 			...
	 			$this->Crud_modal->data_insert("mm_group_members",$createdata);
...
//  new code for interactive case study
	 	...
	 	try{
 			if($this->session->userdata('admin_name')!="" || $this->session->userdata('admin_name')!=null ){
                 ...
                 //echo "hai"; die;
				    $where = "status = 1";
					//$assign_id=$this->input->post('assignorder');
					//$data['topic_list']= $this->Crud_modal->all_data_select('*','mm_interactive_case_study','status=1','t_name desc');
					// if($this->input->post('assignorder')!='')
					// {
	  	...
	  	try
			{
				   if($this->session->userdata('admin_name')!="" || $this->session->userdata('admin_name')!=null )
		     {
				    ...
				    //print_r($data['master_skills_tests']); //die;
				    //mm_interactive_case_study
					$this->load->view('admintempletes/head',$data);
					$this->load->view('admintempletes/header',$data);	
				  	$this->load->view('admintempletes/sidebar',$data);
	  	...
	  	try
			{
				   if($this->session->userdata('admin_name')!="" || $this->session->userdata('admin_name')!=null )
		     {
				...
				//print_r($data['case_map_data']); die;
				    //print_r($data['master_skills_tests']);
				    //mm_interactive_case_study
					$this->load->view('admintempletes/head',$data);
					$this->load->view('admintempletes/header',$data);	
				  	$this->load->view('admintempletes/sidebar',$data);
	  	...
	  	try
			{
				   if($this->session->userdata('admin_name')!="" || $this->session->userdata('admin_name')!=null )
		     {
				...
				//print_r($data['interactive_case_map_edit']);
				//print_r($data['interactive_question']); die;
					$this->load->view('admintempletes/head',$data);
					$this->load->view('admintempletes/header',$data);	
				  	$this->load->view('admintempletes/sidebar',$data);
	 ...
	 public function edit_interactive_case_option_save(){
		try{
			if($this->session->userdata('admin_name')!="" || $this->session->userdata('admin_name')!=null){
				//print_r($_POST);
				//die;
				//$skll_map = implode(',', $this->input->post('option_id')); 
